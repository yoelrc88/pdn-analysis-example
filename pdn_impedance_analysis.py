...
PDN_S2P = "ntwk1.s2p"  # Using existing file as PDN example - ntwk1.s2p has more realistic PDN characteristics
...
ax1.loglog(fGHz, np.abs(Zin_total), 'b-', linewidth=3)
...
# 2. Impedance Phase
ax2.semilogx(fGHz, np.angle(Zin_total, deg=True), 'g-', linewidth=3)
...
# 3. Return Loss (calculated from impedance)
# Calculate reflection coefficient: Γ = (Z - Z0) / (Z + Z0)
Z0 = 1.0  # Reference impedance
...
plt.loglog(fGHz, np.abs(Zin_total), 'b-', linewidth=4)
...
# Impedance phase
plt.figure(figsize=(10, 8))
plt.semilogx(fGHz, np.angle(Zin_total, deg=True), 'g-', linewidth=4)
plt.title("Driving-Point ∠Z at Die (ref 1 Ω)", fontsize=16, fontweight='bold')
plt.xlabel("Frequency [GHz]", fontsize=14)
...
for f_target in freq_points:
    if f_target >= fGHz[0] and f_target <= fGHz[-1]:
        ...
        phase_at_freq = np.angle(Zin_total[idx], deg=True)
...
# 
# The following cell converts this notebook to a standalone Python script that can be run independently.
...
# Read this notebook
nb = nbformat.read("pdn_impedance_analysis.ipynb", as_version=4)

# Convert to Python script
script, _ = ScriptExporter().from_notebook_node(nb)

# Write the script to file
with open("pdn_impedance_analysis.py", "w") as f:
    f.write(script)

print("Exported notebook to pdn_impedance_analysis.py")
print("You can now run: python pdn_impedance_analysis.py")
